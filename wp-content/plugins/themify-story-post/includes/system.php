...
class Themify_Story_Post {
	...
	/*
	 * Iterator for story instances on a given page
	 */
	/*var $instance;*/
	var $instance;
	var $pid = 'themify-story-posts';

	/**
	 * Multi-d array containing information about available story themes
	 */
	var $themes = array();
	/**
	 * Currently active story theme */
	var $active_theme;
	...
	/**
	 * Used internally for storing a copy of $wp_query on story archive pages */
	var $original_query;

	public function __construct( $args=array() ) {
		$this->dir = isset($args['dir'])?$args['dir']:THEMIFY_STORY_POST_DIR;
		$this->url = isset($args['url'])?$args['url']:THEMIFY_STORY_POST_URI;
		$this->version = isset($args['version'])?$args['version']:THEMIFY_STORY_POST_VERSION;
		$this->actions();
	}
 	...
 	public function actions() {
		...
		if( THEMIFY_STORY_POSTS_COMPAT_MODE == true ) {
			add_filter( 'builder_is_story_active', '__return_true' );
			return;
		}
		...
		add_shortcode( 'themify_story_posts', array( $this, 'shortcode' ) );
		...
		$this->register_theme( array(
			'id' => 'stack',
			'label' => __( 'Stack', 'themify-story-posts' ),
			'url' => $this->url . 'themes/stack',
			'dir' => $this->dir . 'themes/stack',
		) );
		do_action( 'themify_story_posts_themes', $this );
		$this->active_theme = $this->themes[ $this->get_option( 'theme' ) ];

		// load custom functions.php from the active story theme
	...
	public function enqueue() {
		wp_enqueue_script( 'themity-story-post', $this->url . 'assets/scripts.js', array( 'jquery' ), $this->version, true );
		wp_enqueue_style( 'themity-story-post', $this->url . 'assets/style.css', null, $this->version );
	}

	public function admin_enqueue() {
		/*
		wp_enqueue_script( 'themity-story-post' );
		wp_enqueue_style( 'themity-story-post-admin', $this->url . 'assets/admin.css' );
		*/
	}
	...
	public function register_module( $ThemifyBuilder ) {
 	...
 	public function admin() {
		if( is_admin() ) {
			...
			new Themify_Story_Posts_Admin();
		}
	}

	public function archive_template( $archive_template ) {
     global $post,$StoryPostObject,$args;

     if ( is_post_type_archive ( 'story' ) ) {
     	$StoryPostObject=$this;
			$args = $this->parse_atts();
			$archive_template = $this->get_theme_dir() . '/archive-story.php';
/*
			...
			// render the story items and append the result to the_content
			$content .= $this->get_template( 'archive-story', $args );

			// to be sure the output is not modified again, remove the filter
			//remove_filter( 'the_content', array( $this, 'output_story_template' ) );
  */
     }
	...
	public function template_redirect() {
	  ...
	  if( is_tax( 'story-category' ) && '' != $this->get_option( 'index_page_template' ) ) {
			...
			add_filter( 'the_content', array( $this, 'index_story_template' ) );

		} elseif( is_singular( 'story' ) && $this->get_option( 'single_page_template' ) ) {
			...
			add_filter( 'the_content', array( $this, 'single_story_template' ) );
			add_filter( 'the_title', array( $this, 'single_story_title' ), 10, 2 );

		}
	}

	public function index_story_template( $content ) {
		global $post;
		...
		if( $post->post_type == 'page' && is_main_query() ) {
			...
			// render the story items and append the result to the_content
			$content .= $this->get_template( 'story-loop', $args );
	...
	public function single_story_template( $content ) {
		global $post;
		...
		if( $post->post_type == 'page' && is_main_query() ) {
			...
			// render the story item and append the result to the_content
			$content .= $this->get_template( 'single-story', $args );
	 ...
	 * @return string
	 */
	public function single_story_title( $title, $id ) {
		global $post;
	...
	function register() {
		$cpt = array(
			...
			'rewrite' => apply_filters( 'themify_story_post_rewrite', $this->get_option( 'story_permalink' ) )
		);
		register_post_type( 'story', apply_filters( 'themify_story_post_args', array(
			'labels' => array(
				'name' => $cpt['plural'],
	...
	public function shortcode( $atts, $content = '' ) {
		...
		if( $query ) {
			...
			return $this->locate_template( 'story-loop' );
		}
	...
	public function get_post_category_classes( $post_id = null ) {
		...
		$categories = wp_get_object_terms( $post_id, 'story-category' );
		$class      = '';
	...
	function get_gallery_images( $field = 'gallery_shortcode' ) {
		$gallery_shortcode = tpp_story_get( $field );
	...
	public function parse_atts( $atts = array() ) {
		...
		if( tpp_is_story_category() || is_post_type_archive ( 'story' ) ) {
		...
		$defaults = apply_filters( 'themify_story_posts_default_atts', $defaults );
		return apply_filters( "themify_story_atts", shortcode_atts( $defaults, $atts ) );
	}
	...
	public function get_options() {
		if( null == $this->options ) {
			$this->options = wp_parse_args( get_option( 'themify_story_posts', array() ), $this->get_default_options() );
		}
	...
	public function get_default_options() {
		return apply_filters( 'themify_story_posts_default_options', array(
			'theme' => 'stack',
			'layout' => 'masonry',
			'enable_masonry' => 'yes',
			'index_display' => 'none',
	 ...
	 * @since 1.0.0
	 */
	public function is_story_category( $term = null ) {
		return isset( $this->original_query ) && $this->original_query->is_tax( 'story-category', $term );
	}
	 ...
	 * @since 1.0.0
	 */
	public function is_story_single( $slug = null ) {
		if( isset( $this->original_query ) && $this->original_query->is_singular( 'story' ) ) {
			if( $slug == null ) {
				return true;
			} else {

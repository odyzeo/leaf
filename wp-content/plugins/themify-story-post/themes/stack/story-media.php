...
<?php if ( $image == 'yes' ) : ?>
	...
	<?php do_action( 'themify_story_post_before_image' ); ?>

	<?php
	if ( tpp_story_get( 'video_url' ) != '' ) : ?>

		<figure class="post-image">
			<?php
				global $wp_embed;
				echo $wp_embed->run_shortcode('[embed]' . esc_url( tpp_story_get( 'video_url' ) ) . '[/embed]');
			?>
		</figure>
	...
	<?php else: ?>
		...
		if ( 'yes' == $use_original_dimensions ) {
			$image_w = tpp_story_get( 'image_width' );
			$image_h = tpp_story_get( 'image_height' );
		}
	...
	<?php do_action( 'themify_story_post_after_image' ); ?>

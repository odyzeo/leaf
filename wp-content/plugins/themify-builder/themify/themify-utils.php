 ...
 * @since 1.2.2
 */
define( 'THEMIFY_DIR', plugin_dir_path( __FILE__ ) );
define( 'THEMIFY_URI', plugin_dir_url( dirname( __FILE__ ) ) . 'themify' );
		...
		<div id="content">
            ...
            <div id="transfer" class="maintab">
                ...
                <div id="transfer-import" class="subtab">
                    ...
                    $baseurl = add_query_arg( 'export', 'true', $baseurl );
	            	?>
...
echo '<div id="themify-meta-box-tabs">';		
                                ...
                                <div class="themify_field_row clearfix <?php echo $toggle_class; ?>" <?php echo $ext_attr; ?>>
                                    ...
                                    <?php foreach($themify_fi_sizes as $option): ?>
                                            <?php if($option['value'] == $meta_value){ $selected = "selected='selected'"; } else { $selected = ""; } ?>
                                             <option value="<?php echo esc_attr( $option['value'] ); ?>" <?php echo $selected; ?>><?php echo esc_html( $option['name'] ); ?></option>
                                            <?php endforeach; ?>   
                                        </select>
                            ...
                            <input type="hidden" name="<?php echo $meta_box['name']; ?>_noncename" id="<?php echo $meta_box['name']; ?>_noncename" value="<?php echo wp_create_nonce( plugin_basename(__FILE__) ); ?>" />
                               ...
                               <div class="themify_field_row clearfix <?php echo $toggle_class; ?>" <?php echo $ext_attr; ?>>
                                    ...
                                    <div class="themify_field">
                                        ...
                                        ?>
                                        <textarea name="<?php echo $meta_box['name']; ?>" size="55" rows="4" class="themify_input_field themify-gallery-shortcode-input <?php echo esc_attr( $class ); ?>"><?php echo $meta_value; ?></textarea>
	...
	<div class="themify_field_row clearfix <?php echo $toggle_class; ?>" <?php echo $ext_attr; ?>>
		...
		<div class="themify_field">
                            	...
                            	?>
                            	<input type="radio" name="<?php echo $meta_box['name']; ?>" id="<?php echo $meta_box['name']; ?>-<?php echo esc_attr( $option['value'] ); ?>" value="<?php echo esc_attr( $option['value'] ); ?>" <?php echo $radio_selected; ?> /> <label for="<?php echo $meta_box['name']; ?>-<?php echo esc_attr( $option['value'] ); ?>" class="selectit"><?php echo esc_html( $option['name'] ); ?></label>
                            	<?php } ?>
...
function themify_meta_field_date( $args, $call_before_after = true, $echo = true ) {
	wp_enqueue_style( 'themify-datetimepicker-css', THEMIFY_URI . '/css/jquery-ui-timepicker.css', array(), false );
			...
			foreach($themify_write_panels as $write_panel){
				...
				foreach($write_panel['options'] as $meta_box) {
					...
					if('multi' == $meta_box['type']){
						// Grouped fields
						foreach ($meta_box['meta']['fields'] as $field) {
							$data = isset( $_POST[$field['name']] ) ? $_POST[$field['name']] : '';
	...
	if ( is_multisite() && !is_upload_space_available() ) {
		...
		if( '' != $fallback )
			...
			<?php } ?>
				...
				?>
				<div class="plupload-upload-uic hide-if-no-js <?php echo $classes; ?>" <?php echo $datas; ?> id="<?php echo esc_attr( $id ); ?>plupload-upload-ui">

					<input id="<?php echo esc_attr( $id ); ?>plupload-browse-button" type="button" value="<?php echo $label; ?>" class="button plupload-button plupload-<?php echo $id ?>" />
	...
	function themify_generic_slider_controls( $prefix ) {
		...
		$output = '<p>
						...
						foreach($auto_options as $name => $val){
							$output .= '<option value="' . esc_attr( $val ) . '" ' . selected($data[$prefix.'autoplay'], $data[$prefix.'autoplay']? $val: 4000, false) . '>' . esc_html( $name ) . '</option>';
						}
		$output .= '	</select>
					</p>';
		...
		$output .= '<p>
						...
						foreach($speed_options as $name => $val){
							$output .= '<option value="' . esc_attr( $val ) . '" ' . selected($data[$prefix.'transition_speed'], $data[$prefix.'transition_speed']? $val: 500, false) . '>' . esc_html( $name ) . '</option>';
						}
		$output .= '	</select>
					</p>';
...
/**
 * Replaces MediaElement base script and loader.
 *
 * @since 1.2.5
 */
...
add_action( 'wp_enqueue_scripts', 'themify_enqueue_framework_assets' );
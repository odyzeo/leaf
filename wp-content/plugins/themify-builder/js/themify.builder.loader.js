    ...
    $(document).ready(function () {
        function remove_tinemce() {
            if (tinymce !== undefined && tinyMCE) {
                ...
                tinyMCEPreInit.mceInit['tb_lb_hidden_editor']['plugins'] = 'charmap,colorpicker,hr,lists,media,paste,tabfocus,textcolor,fullscreen,wordpress,wpautoresize,wpeditimage,wpemoji,wpgallery,wpdialogs,wptextpattern,wpview';
                ...
                $('#wp-tb_lb_hidden_editor-editor-container,#wp-tb_lb_hidden_editor-editor-tools,#wp-link-backdrop,#wp-link-wrap').remove();
            }
        }
        var builderLoader = false,
            $body = $('body');
        if (wp === undefined || wp.customize === undefined) {
            var link = '<a class="themify_builder_turn_on js-turn-on-builder" href="javascript:void(0);"><span class="dashicons dashicons-edit"></span>' + tbLoaderVars.turnOnBuilder + '</a>',
                    builder = document.querySelectorAll('.themify_builder_content:not(.not_editable_builder)');
            for (var i = 0, len = builder.length; i < len; ++i) {
                builder[i].insertAdjacentHTML('afterend', link);
            }
        }
        ...
        $body.one('click.tbloader', '.toggle_tb_builder a:first, a.js-turn-on-builder', function (e) {
            e.preventDefault();
            var post_id = $('#wpadminbar .themify_builder_front_icon').data('id');
            ...
            if (!builderLoader) {
                setTimeout(function () {
                    ...
                    $('.themify_builder_front_icon').parent()[0].insertAdjacentHTML('beforeend', tbLoaderVars.progress);
                }, 1);
            }

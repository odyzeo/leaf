...
class Themify_Builder_Include extends Themify_Builder_Component_Base{
	...
	public function __construct( Themify_Builder $builder ) {
            ...
            if(Themify_Builder_Model::is_premium()){
                if(Themify_Builder_Model::is_frontend_editor_page() && file_exists(THEMIFY_BUILDER_CLASSES_DIR . '/premium/class-themify-builder-revisions.php')){
	...
	public function parallax_elements_fields( $fields ) {
		...
		$new_fields = array(
			...
			array(
				...
				'wrap_with_class' => !$is_premium?'themify_builder_lite':'tb_parrallax'
			),
			array(
				...
				'wrap_with_class' => !$is_premium?'themify_builder_lite':'custom_parallax_scroll_reverse tb_parrallax'
			),
			array(
				...
				'wrap_with_class' => !$is_premium?'themify_builder_lite':'custom_parallax_scroll_fade tb_parrallax'
			),
			array(
	...
	public function do_slider_background( $row_or_col, $order, $builder_type = 'row' ) {
		if ( empty( $row_or_col['styling']['background_slider'] ) 
			|| empty( $row_or_col['styling']['background_type'] ) || 'slider' !== $row_or_col['styling']['background_type'] ) {
			return false;
		}
		
		$size = isset( $row_or_col['styling']['background_slider_size'] )
			? $row_or_col['styling']['background_slider_size'] : false;

		if ( $images = Themify_Builder_Model::get_images_from_gallery_shortcode(
			$row_or_col['styling']['background_slider'] ) ) :
			$bgmode = ! empty( $row_or_col['styling']['background_slider_mode'] )
				? $row_or_col['styling']['background_slider_mode'] : 'fullcover';
			
			if ( ! $size ) {
				$size = Themify_Builder_Model::get_gallery_param_option(
					$row_or_col['styling']['background_slider'], 'size' );

				if ( ! $size ) { $size = 'large'; }
			}
                        if($builder_type==='sub_column'){
				$builder_type = 'column';
			}
			printf( '<div id="%s-slider-%s" class="%s-slider" data-bgmode="%s">'
				, $builder_type, $order, $builder_type, $bgmode ); ?>

			<ul class="row-slider-slides clearfix">
				<?php foreach ( $images as $index => $image ) {
					$img_data = wp_get_attachment_image_src( $image->ID, $size );

					printf( '<li data-bg="%s" data-bg-alt="%s"><a class="row-slider-dot" data-index="%s"></a></li>'
						, esc_url( themify_https_esc( $img_data[0] ) )
		...
		<?php echo '</div>'; endif; // images 
	}

	public function background_styling($builder_id,$row,$order,$type){
		
		// Background cover color
		$return = $this->do_color_overlay($row['styling']);
		//check responsive
		if(!$return){
			$breakpoints = themify_get_breakpoints('all',true);
			foreach($breakpoints as $k=>$v){
				if(!empty($row['styling']['breakpoint_'.$k])){
					$return = $this->do_color_overlay($row['styling']['breakpoint_'.$k]);
					if($return){
						break;
					}
				}
	...
	public function subrow_styling_fields($fields){
		...
		$props = array(
			...
			array(
				...
				'option_js' => true,
				'wrap_with_class' =>(!$is_premium?'themify_builder_lite hide_opacity':'')
			),
			// Background Slider
			array(
			...
			array(
				...
				'meta' => array(
						...
						array('value' => 'fullcover', 'name' => __('Fullcover', 'themify')),
						array('value' => 'builder-parallax-scrolling', 'name' => __('Parallax Scrolling', 'themify'),'disable'=>!$is_premium),
				...
				'wrap_with_class' => 'reponive_disable tb-group-element tb-group-element-image tb-image-options',
                                'binding' => array(
                                        'repeat-none' => array(
			...
			array(
				...
				'default'=>'color',
				'option_js' => true,
				'wrap_with_class' => 'tb-overlay-element'.(!$is_premium?' themify_builder_lite':'')
			),
			array(
				'id' => 'cover_color',
				'type' => 'color',
				'label' => '',
				'class' => 'small',
				'wrap_with_class' => 'tb-group-element tb-group-element-color'.(!$is_premium?' themify_builder_lite':''),
				'prop' => 'background-color',
				'selector' => $key.'>.builder_row_cover'
			),
			array(
				'id' => 'cover_gradient',
				'type' => 'gradient',
				'label' =>'',
				'wrap_with_class' => 'tb-group-element tb-group-element-cover_gradient'.(!$is_premium?' themify_builder_lite':''),
				'prop' => 'background-image',
				'selector' =>$key.'>.builder_row_cover'
			),
			array(
			...
			array(
				...
				'label' => '',
				'class' => 'small',
				'wrap_with_class' => 'tb-group-element tb-group-element-hover_color'.(!$is_premium?' themify_builder_lite':''),
				'prop' => 'background-color',
			...
			array(
				...
				'type' => 'gradient',
				'label' =>'',
				'wrap_with_class' => 'tb-group-element tb-group-element-hover_gradient'.(!$is_premium?' themify_builder_lite':''),
				'prop' => 'background-image',
				'selector' => $key.'>.builder_row_cover::before'
			)
		);

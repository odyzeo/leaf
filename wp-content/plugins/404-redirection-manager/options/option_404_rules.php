...
$misc = SRP_PLUGINS::get_misc();
$app = SRP_PLUGINS::get_app();
...
if($request->post('add')!='' || $request->post('edit','int')>0)
{
    ...
    if(($request->post('edit','int')>0 && $wpdb->get_var("select ID from " . SR_database::WP_SEO_Redirection() . " where redirect_from='$redirect_from' and cat='404rule' and blog='" . get_current_blog_id() . "'")==$request->post('edit','int') ) || ($redirect_from=='' && $wpdb->get_var("select redirect_from_type from " . SR_database::WP_SEO_Redirection() . " where redirect_from_type='$redirect_from_type' and cat='404rule'")!=$redirect_from_type ) || ( $redirect_from!='' && $wpdb->get_var("select redirect_from from " . SR_database::WP_SEO_Redirection() . " where redirect_from='$redirect_from' and cat='404rule'")!=$redirect_from) )
    {
        ...
        if($request->post('edit')!='')
        {
            ...
            $wpdb->query($wpdb->prepare(" update " .  SR_database::WP_SEO_Redirection() . " set redirect_from=%s,redirect_to=%s,redirect_type=%s,redirect_from_type=%s , regex=%s,enabled=%s  where blog=%s and ID=%d", $redirect_from, $redirect_to, $redirect_type, $redirect_from_type, $regex, $enabled, get_current_blog_id(), $ID));
        ...
        {
            $cat='404rule';
            $wpdb->query($wpdb->prepare(" insert into " . SR_database::WP_SEO_Redirection() . "(redirect_from,redirect_to,redirect_type,cat,redirect_to_type,redirect_from_type,regex,enabled,blog) values(%s,%s,%s,'404rule','Page',%s,%s,%s,%d) ", $redirect_from, $redirect_to, $redirect_type, $redirect_from_type, $regex, $enabled, get_current_blog_id()));
            SR_redirect_manager::clear_fixed_404($redirect_from, $regex);
...
if($request->post('sel_items')!='')
{
    ...
    $wpdb->query($wpdb->prepare("delete from " . SR_database::WP_SEO_Redirection() . " where blog=%s and ID in (%s)", get_current_blog_id(), $IDs));

    ...
    class clogica_SR_redirect_cache {
        ...
        public function add_redirect($post_id,$is_redirected,$redirect_from,$redirect_to,$redirect_type=301)
        {
           if(self::cache_enabled()){
            ...
            $wpdb->query($wpdb->prepare("insert IGNORE into $table_name(ID,is_redirected,redirect_from,redirect_to,redirect_type,blog) values(%s,%s,%s,%s,%s,%s); ",$post_id, $is_redirected, $redirect_from, $redirect_to, $redirect_type, get_current_blog_id()));
           }
        }
        ...
        public function fetch_redirect($post_id)
        {
            global $wpdb;
            $table_name = SR_database::WP_SEO_Cache();
            return $wpdb->get_row($wpdb->prepare("select *  from  $table_name where blog='" . get_current_blog_id() . "' and ID=%s; ", $post_id ));
        }
        ...
        public function redirect_cached($post_id)
        {
            ...
            if($redirect != null && $redirect->redirect_from==SRP_PLUGINS::get_request()->get_current_relative_url() && self::cache_enabled())
            {                
        ...
        public static function free_cache($force=0)
        {
            ...
            global $wpdb;
            $table_name = SR_database::WP_SEO_Cache();
            $wpdb->query($wpdb->prepare(" delete from $table_name where blog=%s ",get_current_blog_id() ));
            if(self::show_notifications() && $force!=2)
            SRP_PLUGINS::get_app()->echo_message("<b>All cached redirects are deleted!</b>");
            }
        }
        ...
        /* ----------------------------------------------- */
         ...
         {
             return (intval(SRP_PLUGINS::get_options()->read_option_value("cache_enable"))!=0);
         }

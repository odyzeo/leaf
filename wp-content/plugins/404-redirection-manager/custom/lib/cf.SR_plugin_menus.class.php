...
class SR_plugin_menus
{
    ...
    {
        self::$tabs = SRP_PLUGINS::get_tabs();
        self::$tabs->init();
    ...
    public static function side_menu_content()
    {
        ...
        echo '<div class="wrap" style="direction: ltr"><div class="title_logo">' . SR_PLUGIN_NAME . '</div>';
		echo '<h4>Upgrade to <a target="_blank" href="http://www.clogica.com/product/seo-redirection-premium-wordpress-plugin">pro version</a> and empower your site SEO,<strong style="color:green"> Now 39% off</strong></h4>';
        if(SRP_PLUGINS::get_app()->search_plugins('redirection','404-301-seo-redirection')!='')
        {
           // SRP_PLUGINS::get_app()->echo_message("<b>There is another redirect plugin (" . SRP_PLUGINS::get_app()->search_plugins('redirection','404-redirection-manager') . ") installed, please deactivate it to avoid conflict</b>",'error');
        }

        if(SRP_PLUGINS::get_app()->search_plugins('redirect-to-homepage')!='')
        {
        //    SRP_PLUGINS::get_app()->echo_message("<b>There is another redirect plugin (" . SRP_PLUGINS::get_app()->search_plugins('redirect-to-homepage') . ") installed, please deactivate it to avoid conflict</b>",'error');
        }


        if(SRP_PLUGINS::get_options()->read_option_value('404_plugin_status')=='0')
        {
            SRP_PLUGINS::get_app()->echo_message("<b>The plugin is currently disabled, go to the options tab and enable it!</b>",'warning');
        }
		
		 if(SRP_PLUGINS::get_options()->read_option_value('404_plugin_status')=='2')
        {
            SRP_PLUGINS::get_app()->echo_message("<b>The plugin is currently disabled <strong style=\"color:yellow; font-size:17px\">for admin</strong>, go to the options tab and enable it!</b>",'warning');
        }
    ...
    public static function setup_redirect_menu($permalink,$outcome='echo')
    {
        ...
        <div style=\"text-align:center;\">
              <input onclick=\"window.location='options-general.php?page=" . SRP_PLUGINS::get_app()->get_plugin_slug() . "&SR_tab=redirect_manager&add=1$grpwhere&link=$link&return=$return'\" type=\"button\" value=\"Redirect\" class=\"peter-river-flat-button\" style=\"width:90%; margin-bottom:10px; margin-right:10px \" id=\"publish\" name=\"publish\">
              <input onclick=\"window.location='options-general.php?page=" . SRP_PLUGINS::get_app()->get_plugin_slug() . "&SR_tab=redirect_manager&add=1$grpwhere&link=$link&return=$return&post_operation=draft&post_operation_id=". get_the_ID() ."'\" type=\"button\" value=\"Redirect & Draft\" class=\"orange-flat-button\" style=\"width:90%; margin-bottom:10px; margin-right:10px \" id=\"publish\" name=\"publish\">
              <input onclick=\"window.location='options-general.php?page=" . SRP_PLUGINS::get_app()->get_plugin_slug() . "&SR_tab=redirect_manager&add=1$grpwhere&link=$link&return=$return&post_operation=trash&post_operation_id=". get_the_ID() ."'\" type=\"button\" value=\"Redirect & Trash\" class=\"pomegranate-flat-button\" style=\"width:90%; margin-bottom:10px; margin-right:10px \" id=\"publish\" name=\"publish\">
        </div>
        ";
    ...
    public static function redirected_menu($permalink, $redirect, $outcome='echo')
    {
        ...
        $html="
            <div style=\"color: #23527c;\">
              ...
              <br/>
              <div style=\"text-align:center;\">
              <input onclick=\"window.location='options-general.php?page=" . SRP_PLUGINS::get_app()->get_plugin_slug() . "&SR_tab=redirect_manager&edit=$rid&return=$return'\" type=\"button\" value=\"Update\" class=\"peter-river-flat-button\" style=\"width:40%; margin-bottom:10px; margin-right:10px \" id=\"publish\" name=\"publish\">
                 <input onclick=\"window.location='options-general.php?page=" . SRP_PLUGINS::get_app()->get_plugin_slug() . "&SR_tab=redirect_manager&redirect_manager_tab=redirects&del=$rid&return=$return'\" type=\"button\" value=\"Delete\" class=\"pomegranate-flat-button\" style=\"width:40%; margin-bottom:10px\" id=\"publish\" name=\"publish\">
              </div>
        ";

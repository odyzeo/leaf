...
foreach( array( 'image', 'audio', 'postmeta', 'post_id_info', 'multi', 'date', 'video', 'color', 'dropdown', 'dropdownbutton', 'textbox', 'textarea', 'checkbox', 'separator', 'layout', 'radio', 'gallery_shortcode','wpeditor' ) as $field_type ) {
...
function themify_meta_field_multi( $args ) {
	global $post;
	extract( $args, EXTR_OVERWRITE );
...
function themify_meta_field_wpeditor( $args, $meta_box ) {
	extract( $args, EXTR_OVERWRITE );
	$meta_box = wp_parse_args( $meta_box, array(
		'size' => 55,
		'rows' => 4,
  ...
  $html='';
	...
	if( isset( $meta_box['before'] ) && $meta_box['before'] ) echo $meta_box['before'];
  
  wp_editor( esc_textarea( $meta_value ), esc_attr( $meta_box['name'] ), array(
      'wpautop'       => $meta_box['wpautop'],
      'media_buttons' => $meta_box['media_buttons'],
      'textarea_name' => $meta_box['name'],
      'textarea_rows' => $meta_box['rows'],
      'teeny'         => $meta_box['teeny']
  ) );
  ...
  */
	...
	echo themify_meta_field_get_label($html, $meta_box);

	if ( isset( $meta_box['description'] ) ) {
		echo themify_meta_field_get_description( $meta_box['description'] );
	}

	if( isset( $meta_box['after'] ) && $meta_box['after'] )  echo $meta_box['after'];
...
function themify_meta_field_assignments( $args ) {
	...
	?>
	<div class="themify_field_row clearfix <?php echo esc_attr( $toggle_class ); ?>" <?php echo esc_attr( $ext_attr ); ?> <?php if ( ! empty( $data_hide ) ) : echo 'data-hide="' . esc_attr( $data_hide ) . '"'; endif; ?>>
		<div class="themify_field_title"><?php echo esc_html( $field['title'] ); ?></div>
		<div class="themify_field">
	<?php
		...
		$taxonomies = apply_filters( 'themofy_assignments_taxonomies', get_taxonomies( array( 'public' => true ) ) );

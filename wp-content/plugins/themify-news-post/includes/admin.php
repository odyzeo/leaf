...
class Themify_News_Posts_Admin {

	var $options;
	var $post_type = 'news';
	var $tax = 'news-category';
	...
	public function __construct() {
		...
		add_action( 'init', array( $this, 'setup_news_metabox' ) );
		add_action( 'admin_menu', array( $this, 'setup_options_page' ), 100 );
	}

	public function setup_options_page() {
		add_submenu_page( 'edit.php?post_type=news', __( 'News Options', 'themify-news-posts' ), __( 'News Options', 'themify-news-posts' ), 'manage_options', 'themify-news-posts', array( $this, 'create_admin_page' ) );
	}

	public function setup_news_metabox() {
		add_filter( 'themify_do_metaboxes', array( $this, 'themify_do_metaboxes' ) );
	}

	public function themify_do_metaboxes( $metaboxes ) {
		global $themify_news_posts, $pagenow;

		$news_options = array(
			array(
				'name'		=> __( 'News Options', 'themify-news-posts' ),
				'id' 		=> 'news-options',
				'options' 	=> include $themify_news_posts->dir . 'includes/config.php',
				'pages'		=> 'news'
			)
		);
		if( file_exists( $themify_news_posts->locate_template( 'config' ) ) ) {
			$news_options[] = include( $themify_news_posts->locate_template( 'config' ) );
		}
		$news_options = apply_filters( "themify_news_post_options", $news_options );

		return array_merge( $news_options, $metaboxes );
	}

    public function create_admin_page() {
		global $themify_news_posts;

		$this->options = $themify_news_posts->get_options();
		?>
		<div class="wrap">
			<?php screen_icon(); ?>
			<h2><?php _e( 'Themify News Posts', 'themify-news-posts' ); ?></h2>           
			<form method="post" action="options.php">
				<?php
				// This prints out all hidden setting fields
				settings_fields( 'themify_news_posts' );   
				do_settings_sections( 'themify-news-posts' );
				submit_button(); 
				?>
			</form>
		</div>
	...
	public function page_init() {        
		register_setting(
			'themify_news_posts', // Option group
			'themify_news_posts' // Option name
		);

		add_settings_section(
			'tpp-news-general', // ID
			__( 'General Settings', 'themify-news-posts' ), // Title
			null, // Callback
			'themify-news-posts' // Page
		);

		add_settings_section(
			'tpp-news-index', // ID
			__( 'News Index Options', 'themify-news-posts' ), // Title
			null, // Callback
			'themify-news-posts' // Page
		);
		...
		add_settings_section(
			'tpp-news-single', // ID
			__( 'News Single Options', 'themify-news-posts' ), // Title
			null, // Callback
			'themify-news-posts' // Page
		);

		add_settings_section(
			'tpp-news-permalink', // ID
			__( 'News Permalink', 'themify-news-posts' ), // Title
			null, // Callback
			'themify-news-posts' // Page
		);
		...
		add_settings_field(
			'theme', // ID
			__( 'Theme', 'themify-news-posts' ), // Title 
			array( $this, 'theme_callback' ), // Callback
			'themify-news-posts', // Page
			'tpp-news-general' // Section
		);

		add_settings_field(
			'index_page_template', // ID
			__( 'Page Template', 'themify-news-posts' ), // Title 
			array( $this, 'page_select_field' ), // Callback
			'themify-news-posts', // Page
			'tpp-news-index', // Section
			array( 'id' => 'index_page_template' )
		);

		add_settings_field(
			'enable_masonry', // ID
			__( 'Enable Masonry', 'themify-news-posts' ), // Title 
		...
		add_settings_field(
			'layout', // ID
			__( 'News Layout', 'themify-news-posts' ), // Title 
		...
		add_settings_field(
			...
			array( $this, 'page_select_field' ), // Callback
			'themify-news-posts', // Page
			'tpp-news-single', // Section
			array( 'id' => 'single_page_template' )
		);

		add_settings_field(
			'index_display', // ID
			__( 'Display Content', 'themify-news-posts' ), // Title 
		...
		add_settings_field(
			'index_hide_title', // ID
			__( 'Hide Title', 'themify-news-posts' ), // Title 
			array( $this, 'yes_no_select' ), // Callback
			'themify-news-posts', // Page
			'tpp-news-index', // Section
		...
		add_settings_field(
			...
			__( 'Unlink News Title', 'themify-news-posts' ), // Title 
			array( $this, 'yes_no_select' ), // Callback
			'themify-news-posts', // Page
			'tpp-news-index', // Section
		...
		add_settings_field(
			'index_hide_meta', // ID
			__( 'Hide News Meta', 'themify-news-posts' ), // Title 
			array( $this, 'yes_no_select' ), // Callback
			'themify-news-posts', // Page
			'tpp-news-index', // Section
		...
		add_settings_field(
			'index_hide_date', // ID
			__( 'Hide News Date', 'themify-news-posts' ), // Title 
			array( $this, 'yes_no_select' ), // Callback
			'themify-news-posts', // Page
			'tpp-news-index', // Section
		...
		add_settings_field(
			'index_hide_image', // ID
			__( 'Hide News Image', 'themify-news-posts' ), // Title 
			array( $this, 'yes_no_select' ), // Callback
			'themify-news-posts', // Page
			'tpp-news-index', // Section
		...
		add_settings_field(
			...
			__( 'Unlink News Image', 'themify-news-posts' ), // Title 
			array( $this, 'yes_no_select' ), // Callback
			'themify-news-posts', // Page
			'tpp-news-index', // Section
		...
		add_settings_field(
			'index_image_size', // ID
			__( 'Image Size', 'themify-news-posts' ), // Title 
		...
		add_settings_field(
			...
			__( 'Hide News Title', 'themify-news-posts' ), // Title 
			array( $this, 'yes_no_select' ), // Callback
			'themify-news-posts', // Page
			'tpp-news-single', // Section
		...
		add_settings_field(
			'news_permalink', // ID
			__( 'News Base Slug', 'themify-news-posts' ), // Title 
			array( $this, 'news_permalink_callback' ), // Callback
			'themify-news-posts', // Page
			'tpp-news-permalink' // Section
		);
    }
	...
	public function yes_no_select( $args ) {
		?>
		<label>
			<input type="radio" id="" name="themify_news_posts[<?php echo $args['id']; ?>]" value="yes" <?php checked( 'yes', $this->options[$args['id']] ); ?>><?php _e( 'Yes', 'themify-news-posts' ); ?>
		</label>
		<label>
			<input type="radio" id="" name="themify_news_posts[<?php echo $args['id']; ?>]" value="no" <?php checked( 'no', $this->options[$args['id']] ); ?>><?php _e( 'No', 'themify-news-posts' ); ?>
		</label>
		<?php
	}
	...
	public function page_select_field( $args ) {
		wp_dropdown_pages( array(
			...
			'name' => 'themify_news_posts[' . $args['id'] .']',
			'show_option_none' => '&nbsp;'
		) );
	}
	...
	public function theme_callback( $args ) {
		global $themify_news_posts;
		?><select class="regular-text" id="" name="themify_news_posts[theme]">
			<?php foreach( $themify_news_posts->get_themes() as $theme ) : ?>
	...
	public function display_callback( $args ) {
		?><select class="regular-text" id="" name="themify_news_posts[<?php echo $args['id']; ?>]">
			<option value="none" <?php selected( $this->options[$args['id']], 'none' ); ?>> <?php _e( 'None', 'themify-news-posts' ); ?></option>
			<option value="excerpt" <?php selected( $this->options[$args['id']], 'excerpt' ); ?>> <?php _e( 'Excerpt', 'themify-news-posts' ); ?></option>
			<option value="content" <?php selected( $this->options[$args['id']], 'content' ); ?>> <?php _e( 'Content', 'themify-news-posts' ); ?></option>
		</select><?php
	}

	public function layout_callback( $args ) {
		global $themify_news_posts;
		$base_url = $themify_news_posts->url . 'images/layout-icons';
		?>
		<div class="themify-radio-image">
			<label>
				<input type="radio" name="themify_news_posts[layout]" value="masonry" <?php checked( $this->options['layout'], 'masonry' ); ?>>
				<img src="<?php echo $base_url ?>/masonry-view.png" alt="masonry" />
			</label>
			<label>
				<input type="radio" name="themify_news_posts[layout]" value="list-post" <?php checked( $this->options['layout'], 'list-post' ); ?>>
				<img src="<?php echo $base_url ?>/list-post.png" alt="list-post" />
			</label>
			<label>
				<input type="radio" name="themify_news_posts[layout]" value="grid2" <?php checked( $this->options['layout'], 'grid2' ); ?>>
				<img src="<?php echo $base_url ?>/grid2.png" alt="grid2" />
			</label>
			<label>
				<input type="radio" name="themify_news_posts[layout]" value="grid3" <?php checked( $this->options['layout'], 'grid3' ); ?>>
				<img src="<?php echo $base_url ?>/grid3.png" alt="grid3" />
			</label>
			<label>
				<input type="radio" name="themify_news_posts[layout]" value="grid4" <?php checked( $this->options['layout'], 'grid4' ); ?>>
	...
	public function image_size_callback( $args ) {
		?>
		<input class="small-text" id="" name="themify_news_posts[<?php echo $args['id']; ?>][width]" value="<?php echo esc_attr( $this->options[$args['id']]['width'] ) ?>"> <?php _e( 'Width (px)', 'themify-news-posts' ); ?>
		<input class="small-text" id="" name="themify_news_posts[<?php echo $args['id']; ?>][height]" value="<?php echo esc_attr( $this->options[$args['id']]['height'] ) ?>"> <?php _e( 'Height', 'themify-news-posts' ); ?>
		<?php
	}

	public function news_permalink_callback( $args ) {
		?>
		<input class="medium-text" id="" name="themify_news_posts[news_permalink]" value="<?php echo esc_attr( $this->options['news_permalink'] ) ?>">
		<div class="description">
			<?php _e( 'Use only lowercase letters, numbers, underscores and dashes. After changing this, go to permalinks and click "Save changes" to refresh them.', 'themify-news-posts' ); ?>
		</div>
		<?php
	}
	...
	public function get_select() {
		...
		foreach ( $this->taxonomies as $tax ) {
			$options = sprintf( '<option value="">%s %s</option>', __('View All', 'themify-news-posts'),
	...
	function attachment_fields_to_edit( $form_fields, $post ) {
		...
		$form_fields['themify_gallery_featured'] = array(
			'label' => __( 'Larger', 'themify-news-posts' ),
			'input' => 'html',
			'helps' => __('Show larger image in the gallery.', 'themify-news-posts'),

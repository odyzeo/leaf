...
class Themify_Story_Posts_Admin {

	var $options;
	var $post_type = 'story';
	var $tax = 'story-category';
	...
	public function __construct() {
		...
		if( THEMIFY_STORY_POSTS_COMPAT_MODE == true ) {
			return;
		}

		add_action( 'admin_init', array( $this, 'page_init' ) );
		add_action( 'init', array( $this, 'setup_story_metabox' ) );
		add_action( 'admin_menu', array( $this, 'setup_options_page' ), 100 );
	}

	public function setup_options_page() {
		add_submenu_page( 'edit.php?post_type=story', __( 'Story Options', 'themify-story-posts' ), __( 'Story Options', 'themify-story-posts' ), 'manage_options', 'themify-story-posts', array( $this, 'create_admin_page' ) );
	}

	public function setup_story_metabox() {
		add_filter( 'themify_do_metaboxes', array( $this, 'themify_do_metaboxes' ) );
	}

	public function themify_do_metaboxes( $metaboxes ) {
		global $themify_story_posts, $pagenow;

		$story_options = array(
			array(
				'name'		=> __( 'Story Options', 'themify-story-posts' ),
				'id' 		=> 'story-options',
				'options' 	=> include $themify_story_posts->dir . 'includes/config.php',
				'pages'		=> 'story'
			)
		);
		if( file_exists( $themify_story_posts->locate_template( 'config' ) ) ) {
			$story_options[] = include( $themify_story_posts->locate_template( 'config' ) );
		}
		$story_options = apply_filters( "themify_story_post_options", $story_options );

		return array_merge( $story_options, $metaboxes );
	}

    public function create_admin_page() {
		global $themify_story_posts;

		$this->options = $themify_story_posts->get_options();
		?>
		<div class="wrap">
			<?php screen_icon(); ?>
			<h2><?php _e( 'Themify Story Posts', 'themify-story-posts' ); ?></h2>           
			<form method="post" action="options.php">
				<?php
				// This prints out all hidden setting fields
				settings_fields( 'themify_story_posts' );   
				do_settings_sections( 'themify-story-posts' );
				submit_button(); 
				?>
			</form>
		</div>
	...
	public function page_init() {        
		register_setting(
			'themify_story_posts', // Option group
			'themify_story_posts' // Option name
		);

		add_settings_section(
			'tpp-story-general', // ID
			__( 'General Settings', 'themify-story-posts' ), // Title
			null, // Callback
			'themify-story-posts' // Page
		);

		add_settings_section(
			'tpp-story-index', // ID
			__( 'Story Index Options', 'themify-story-posts' ), // Title
			null, // Callback
			'themify-story-posts' // Page
		);
		...
		add_settings_section(
			'tpp-story-single', // ID
			__( 'Story Single Options', 'themify-story-posts' ), // Title
			null, // Callback
			'themify-story-posts' // Page
		);

		add_settings_section(
			'tpp-story-permalink', // ID
			__( 'Story Permalink', 'themify-story-posts' ), // Title
			null, // Callback
			'themify-story-posts' // Page
		);
		...
		add_settings_field(
			'theme', // ID
			__( 'Theme', 'themify-story-posts' ), // Title 
			array( $this, 'theme_callback' ), // Callback
			'themify-story-posts', // Page
			'tpp-story-general' // Section
		);

		add_settings_field(
			'index_page_template', // ID
			__( 'Page Template', 'themify-story-posts' ), // Title 
			array( $this, 'page_select_field' ), // Callback
			'themify-story-posts', // Page
			'tpp-story-index', // Section
			array( 'id' => 'index_page_template' )
		);

		add_settings_field(
			'enable_masonry', // ID
			__( 'Enable Masonry', 'themify-story-posts' ), // Title 
		...
		add_settings_field(
			'layout', // ID
			__( 'Story Layout', 'themify-story-posts' ), // Title 
		...
		add_settings_field(
			...
			array( $this, 'page_select_field' ), // Callback
			'themify-story-posts', // Page
			'tpp-story-single', // Section
			array( 'id' => 'single_page_template' )
		);

		add_settings_field(
			'index_display', // ID
			__( 'Display Content', 'themify-story-posts' ), // Title 
		...
		add_settings_field(
			'index_hide_title', // ID
			__( 'Hide Title', 'themify-story-posts' ), // Title 
			array( $this, 'yes_no_select' ), // Callback
			'themify-story-posts', // Page
			'tpp-story-index', // Section
		...
		add_settings_field(
			...
			__( 'Unlink Story Title', 'themify-story-posts' ), // Title 
			array( $this, 'yes_no_select' ), // Callback
			'themify-story-posts', // Page
			'tpp-story-index', // Section
		...
		add_settings_field(
			'index_hide_meta', // ID
			__( 'Hide Story Meta', 'themify-story-posts' ), // Title 
			array( $this, 'yes_no_select' ), // Callback
			'themify-story-posts', // Page
			'tpp-story-index', // Section
		...
		add_settings_field(
			'index_hide_date', // ID
			__( 'Hide Story Date', 'themify-story-posts' ), // Title 
			array( $this, 'yes_no_select' ), // Callback
			'themify-story-posts', // Page
			'tpp-story-index', // Section
		...
		add_settings_field(
			'index_hide_image', // ID
			__( 'Hide Story Image', 'themify-story-posts' ), // Title 
			array( $this, 'yes_no_select' ), // Callback
			'themify-story-posts', // Page
			'tpp-story-index', // Section
		...
		add_settings_field(
			...
			__( 'Unlink Story Image', 'themify-story-posts' ), // Title 
			array( $this, 'yes_no_select' ), // Callback
			'themify-story-posts', // Page
			'tpp-story-index', // Section
		...
		add_settings_field(
			'index_image_size', // ID
			__( 'Image Size', 'themify-story-posts' ), // Title 
		...
		add_settings_field(
			...
			__( 'Hide Story Title', 'themify-story-posts' ), // Title 
			array( $this, 'yes_no_select' ), // Callback
			'themify-story-posts', // Page
			'tpp-story-single', // Section
		...
		add_settings_field(
			'story_permalink', // ID
			__( 'Story Base Slug', 'themify-story-posts' ), // Title 
			array( $this, 'story_permalink_callback' ), // Callback
			'themify-story-posts', // Page
			'tpp-story-permalink' // Section
		);
    }
	...
	public function yes_no_select( $args ) {
		?>
		<label>
			<input type="radio" id="" name="themify_story_posts[<?php echo $args['id']; ?>]" value="yes" <?php checked( 'yes', $this->options[$args['id']] ); ?>><?php _e( 'Yes', 'themify-story-posts' ); ?>
		</label>
		<label>
			<input type="radio" id="" name="themify_story_posts[<?php echo $args['id']; ?>]" value="no" <?php checked( 'no', $this->options[$args['id']] ); ?>><?php _e( 'No', 'themify-story-posts' ); ?>
		</label>
		<?php
	}
	...
	public function page_select_field( $args ) {
		wp_dropdown_pages( array(
			...
			'name' => 'themify_story_posts[' . $args['id'] .']',
			'show_option_none' => '&nbsp;'
		) );
	}
	...
	public function theme_callback( $args ) {
		global $themify_story_posts;
		?><select class="regular-text" id="" name="themify_story_posts[theme]">
			<?php foreach( $themify_story_posts->get_themes() as $theme ) : ?>
	...
	public function display_callback( $args ) {
		?><select class="regular-text" id="" name="themify_story_posts[<?php echo $args['id']; ?>]">
			<option value="none" <?php selected( $this->options[$args['id']], 'none' ); ?>> <?php _e( 'None', 'themify-story-posts' ); ?></option>
			<option value="excerpt" <?php selected( $this->options[$args['id']], 'excerpt' ); ?>> <?php _e( 'Excerpt', 'themify-story-posts' ); ?></option>
			<option value="content" <?php selected( $this->options[$args['id']], 'content' ); ?>> <?php _e( 'Content', 'themify-story-posts' ); ?></option>
		</select><?php
	}

	public function layout_callback( $args ) {
		global $themify_story_posts;
		$base_url = $themify_story_posts->url . 'images/layout-icons';
		?>
		<div class="themify-radio-image">
			<label>
				<input type="radio" name="themify_story_posts[layout]" value="masonry" <?php checked( $this->options['layout'], 'masonry' ); ?>>
				<img src="<?php echo $base_url ?>/masonry-view.png" alt="masonry" />
			</label>
			<label>
				<input type="radio" name="themify_story_posts[layout]" value="list-post" <?php checked( $this->options['layout'], 'list-post' ); ?>>
				<img src="<?php echo $base_url ?>/list-post.png" alt="list-post" />
			</label>
			<label>
				<input type="radio" name="themify_story_posts[layout]" value="grid2" <?php checked( $this->options['layout'], 'grid2' ); ?>>
				<img src="<?php echo $base_url ?>/grid2.png" alt="grid2" />
			</label>
			<label>
				<input type="radio" name="themify_story_posts[layout]" value="grid3" <?php checked( $this->options['layout'], 'grid3' ); ?>>
				<img src="<?php echo $base_url ?>/grid3.png" alt="grid3" />
			</label>
			<label>
				<input type="radio" name="themify_story_posts[layout]" value="grid4" <?php checked( $this->options['layout'], 'grid4' ); ?>>
	...
	public function image_size_callback( $args ) {
		?>
		<input class="small-text" id="" name="themify_story_posts[<?php echo $args['id']; ?>][width]" value="<?php echo esc_attr( $this->options[$args['id']]['width'] ) ?>"> <?php _e( 'Width (px)', 'themify-story-posts' ); ?>
		<input class="small-text" id="" name="themify_story_posts[<?php echo $args['id']; ?>][height]" value="<?php echo esc_attr( $this->options[$args['id']]['height'] ) ?>"> <?php _e( 'Height', 'themify-story-posts' ); ?>
		<?php
	}

	public function story_permalink_callback( $args ) {
		?>
		<input class="medium-text" id="" name="themify_story_posts[story_permalink]" value="<?php echo esc_attr( $this->options['story_permalink'] ) ?>">
		<div class="description">
			<?php _e( 'Use only lowercase letters, numbers, underscores and dashes. After changing this, go to permalinks and click "Save changes" to refresh them.', 'themify-story-posts' ); ?>
		</div>
		<?php
	}
	...
	public function get_select() {
		...
		foreach ( $this->taxonomies as $tax ) {
			$options = sprintf( '<option value="">%s %s</option>', __('View All', 'themify-story-posts'),
	...
	function attachment_fields_to_edit( $form_fields, $post ) {
		...
		$form_fields['themify_gallery_featured'] = array(
			'label' => __( 'Larger', 'themify-story-posts' ),
			'input' => 'html',
			'helps' => __('Show larger image in the gallery.', 'themify-story-posts'),

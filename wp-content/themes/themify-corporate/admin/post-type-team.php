...
if ( ! class_exists( 'Themify_Team' ) ) {
	...
	class Themify_Team {
		...
		function __construct( $args = array() ) {
			...
			add_filter( 'themify_default_layout_condition', array($this, 'single_team_layout_condition' ), 13 );
			add_filter( 'themify_default_layout', array($this, 'single_team_default_layout' ), 13 );
		}
		...
		function progress_bar( $atts = array() ) {
			...
			return sprintf(
				'<div class="progress-bar"><i class="progress-bar-label">%s</i><span class="progress-bar-bg" data-percent="%s" style="background-color:%s;width:%s;"></span></div>',
				$label,
				$percentage,
				$color,
				$percentage
		...
		function social( $atts = array() ) {
			...
			$out = '
				<a href="' . $atts['link'] . '" title="' . $atts['label'] . '" class="team-social-link">
					<i class="ti-' . $atts['icon'] . '"></i>
				</a>';
		 ...
		 * @return bool
		 */
		function single_team_layout_condition( $condition ) {
		 ...
		 * @return mixed|string
		 */
		function single_team_default_layout( $class ) {
			if ( is_singular( 'team' ) ) {
 ...
 **************************************************************************************************/
$GLOBALS['themify_team'] = new Themify_Team();

...
class Themify {
	...
	function template_redirect() {
		...
		elseif( is_singular('post') || is_singular('portfolio') ) {
			...
			if( !$is_portfolio){
				if ( themify_get('layout', 'default') != 'default' ) {
		...
		elseif ( is_singular( 'team' ) ) {
			$teampre = 'setting-default_team_single_';
                        $this->hide_image = themify_get($teampre.'hide_image');
			$this->layout = themify_check( $teampre.'layout' )?themify_get( $teampre.'layout' ):'sidebar1';
                        $post_image_width = themify_get($teampre.'image_post_width');
                        $post_image_height = themify_get($teampre.'image_post_height');
